...
editor_in_chief = [
    "Qing Gu, Undergraduate, 2018",
    "Hongxu Zhou, Undergraduate, 2019"
]
...
for editor in editors:
    associate_editor.append(editor['editor']['name'] + ", " + editor['editor']['degree'])
...
'''
+
"\\subtitle{\\sffamily " + str(month) + "月刊}"
)
...
for category in categories:
    ...
    for article in categories[category]:
        print("\\item \\href{" + article['doi'] + "}{\\color{blue} \\ul{" + article['title'] + "}}")
...
for category in categories:
    for article in categories[category]:
        print("\\begin{frame}[allowframebreaks]{\\color{black} \\normalsize{\\ul{" + category + "}} \\hfill")
